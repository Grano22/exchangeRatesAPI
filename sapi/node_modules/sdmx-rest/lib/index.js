...
(function() {
  var ApiResources, ApiVersion, AvailabilityMode, AvailabilityQuery, AvailabilityReferences, DataDetail, DataFormat, DataQuery, MetadataDetail, MetadataFormat, MetadataQuery, MetadataReferences, MetadataType, SchemaContext, SchemaFormat, SchemaQuery, SdmxPatterns, Service, UrlGenerator, addHeaders, checkMediaType, checkStatus, fetch, getAvailabilityQuery, getDataQuery, getMetadataQuery, getSchemaQuery, getService, getUrl, guessService, isDataFormat, isFormat, isGenericFormat, isMetadataFormat, isRequestedFormat, isSchemaFormat, promise, request, request2, services, userAgent,
    indexOf = [].indexOf,
  ...
  isFormat = function(input, expected) {
    var i, len, out, ref, v;
    out = false;
    ref = Object.values(expected);
  ...
  isDataFormat = function(format) {
    return isFormat(format, DataFormat);
  };

  isMetadataFormat = function(format) {
    return isFormat(format, MetadataFormat);
  };

  isSchemaFormat = function(format) {
    return isFormat(format, SchemaFormat);
  };

  isGenericFormat = function(format) {
    var formats;
    formats = ['application/xml', 'application/json', 'text/csv', 'text/xml'];
    return indexOf.call(formats, format) >= 0;
  };

  isRequestedFormat = function(requested, received) {
    return requested.indexOf(received) > -1;
  };

  checkMediaType = function(requested, response) {
    var fmt;
    fmt = response.headers.get('content-type');
    fmt = fmt ? fmt.replace(/; version=/, ';version=') : fmt;
    if (!(isDataFormat(fmt) || isMetadataFormat(fmt) || isGenericFormat(fmt) || isSchemaFormat(fmt))) {
  ...
  addHeaders = function(opts, s, type) {
    var headers, key, ref;
    if (opts == null) {
      opts = {};
    }
    ...
    if (!headers.accept) {
      headers.accept = (function() {
        switch (type) {
          case 'data':
            return s.format;
          case 'structure':
            return s.structureFormat;
          case 'schema':
            return s.schemaFormat;
        }
      })();
    }
    if (!headers.accept) {
  ...
  getAvailabilityQuery = function(input) {
  ...
  getUrl = function(query, service) {
    ...
    q = ((query.mode != null) || ((query.flow != null) && (query.references != null)) || ((query.flow != null) && (query.component != null))) ? getAvailabilityQuery(query) : query.flow != null ? getDataQuery(query) : query.resource != null ? getMetadataQuery(query) : query.context != null ? getSchemaQuery(query) : void 0;
    if (!q) {
  ...
  request = function(...params) {
    return request2(...params).then(function(response) {
